...
def render_variables_section(
    ...
    for idx, summary in dataframe_summary.variables.items():
        ...
        if "bottom" in template_variables and template_variables["bottom"] is not None:
            btn = ToggleButton("Mais detalhes", anchor_id=template_variables["varid"])
        ...
        if isinstance(duplicates, list):
            ...
            items.append(
                Duplicate(
                    ...
                    name="Ocorrência(s) mais frequente(s)",
                    anchor_id="duplicates",
                )
            )
    return items
                    ...
                    Container(
                        [
                            ...
                            if zplot != ""
                                ...
                                f"<em>Gráfico de interação ausente no conjunto de dados</em>"
                            )
...
def get_report_structure(config: Settings, summary: BaseDescription) -> Root:
    ...
    with tqdm(
        total=1, desc="Gerar a estrutura do relatório", disable=disable_progress_bar
    ) as pbar:
